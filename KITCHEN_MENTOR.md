...
חוק קיבוץ לפי שימוש (חובה): הטבלה מחולקת לקבוצות לפי רכיבי המנה, וכל קבוצה נפתחת
בשורת כותרת המשתרעת על רוחב הטבלה ("לקציצות", "לרוטב הלימון", "לאורז", "לתיבול העוף").
שמות הקבוצות נגזרים מהמנה עצמה — אין רשימה קבועה. רכיב המשמש בכמה קבוצות חוזר בכל
אחת מהן עם הכמות שלה, גם אם מלח מופיע שלוש פעמים — זה מכוון. סדר הקבוצות = סדר
העבודה במטבח, כך שאפשר לעבוד לפי הטבלה מלמעלה למטה.
בתוך קבוצה אין לחזור על שיוך בסוגריים ("שמן זית", לא "שמן זית (לקציצות)") — הכותרת
כבר אומרת זאת. יוצא דופן יחיד: שני שימושים שונים של אותו רכיב בתוך אותה קבוצה, שאז
מוסיפים תיאור מבחין קצר ("חמאה רכה לבצק" מול "חמאה מומסת להברשה").
